...
class PrettyPrint:
	...
	* to create an instance use:
	...
	# 2000-05-10 ce: New settings. Basically inspired by kwrite (of KDE).
	self.formats = {
	    'all':('<PRE>','</PRE>'),
	    'comment':('<FONT COLOR=#1111CC>','</FONT>'),
	    'keyword':('<FONT COLOR=black><B>','</B></FONT>'),
	    'parameter':('<FONT COLOR=#00FF00>','</FONT>'),
	    'identifier':( lambda x,strip=string.strip:
                           '<A NAME="%s"><FONT COLOR=#AA00AA><B>' % (strip(x)),
	                   '</B></FONT></A>'),
