...
reason is that Apache is a multi process webserver, so the memory option just won't work.  Additionally, if you are running on UNIX,
the whole WebKit process will be running as nobody/apache or whatever user you run apache as.  This means that that user will need
to be able to create, at the minimum, files in the "Sessions" directory.

The only compromise you must make in using this AppServer is that you can't have any Application wide information.  There is no way to
share the information across the application.  This means no hit counters, no chat servers, etc.  You can do those things, but not by the
standard WebKit methods of either stroing the info in the Application instance or using an Applicatino scope Can.  You will need to use
either a shared file, which could get messy in a hurry, or use something like the ZODB.  I plan to implement ZODB as a Kit as soon as
I have time, if some one wants to get started on it, that would be great.  It might also be useful in load balancing.
