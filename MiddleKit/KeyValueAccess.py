	...
	def valuesForNames(self, keys, default=None, defaults=None, forgive=0, includeNames=0):
		...
		if default is not None:
			...
			if len(keys) is not len(defaults):
				raise 'KeyValueAccess', 'Keys and defaults have mismatching lengths (%d and %d).' % (len(keys), len(defaults))
...
def _valueForKeySequence(obj, listOfKeys, default=None):
	'''	This is a recursive function used to implement KeyValueAccess.valueForKeySequence.
		Besides supporting inheritors of KeyValueAccess, this function also supports
	...
	if type(obj) is DictType:
		try:
			...
			if default is None:
				raise 'KeyValueAccess', 'Unknown key (%s) in dictionary.' % listOfKeys[0]
			else:
				return default
	else:
...
# Testing
#
class _t1(KeyValueAccess):
	def foo(self):
		return 1

class _t2(KeyValueAccess):
	def _foo(self):
		return 1

class _t3(KeyValueAccess):
	def foo(self):
		return 1
	def _foo(self):
		return 0

class _t4(KeyValueAccess):
	def foo(self):
		return 1
	def __init__(self):
		self._foo = 0

class _t5(KeyValueAccess):
	def __init__(self):
		self.foo = 0
	def _foo(self):
		return 1

class _t6(KeyValueAccess):
	def __init__(self):
		self.foo = 1
		self._foo = 0

class _t7(KeyValueAccess):
	def handleUnknownGetKey(self, key):
		return 1
