...
class PSPExamplePage(ExamplePage):
	...
	def writeOtherMenu(self):
		...
		self.menuItem(
			...
			self.request().uriWebKitRoot() + 'PSPExamples/View?filename=%s' % os.path.basename(self.request().serverSidePath()))
#string.replace(self.request().serverSidePath(), '\\', '/'))
